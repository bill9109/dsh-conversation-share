...
function insideInteractive(el: HTMLElement): boolean {
  return el.closest(INTERACTIVE_SELECTOR) !== null
}

function hasVisualBox(el: HTMLElement): boolean {
...
function hasBlockChildren(el: HTMLElement): boolean {
  for (const child of el.children) {
    if (WRAPPER_CHILD_TAGS.has(child.tagName)) return true
  }
  return false
}

function isSnapTarget(el: HTMLElement): boolean {
...
function lineEdges(block: HTMLElement, scrollTop: number): Array<{ top: number; bottom: number }> {
  const edges: Array<{ top: number; bottom: number }> = []
...
export class SnapTargets {
  ...
  rebuild(fast = false): void {
    ...
    const push = (el: HTMLElement, top: number, bottom: number, line = false): void => {
    ...
    const rows = Array.from(list.children).filter((child): child is HTMLElement =>
      child instanceof HTMLElement && child.hasAttribute('data-chat-anchor-key'),
    )
    ...
    for (const row of rows) {
      for (const el of row.querySelectorAll<HTMLElement>(
        'div, p, pre, ul, ol, li, table, h1, h2, h3, h4, h5, h6, blockquote, button, a',
      )) {
    ...
    if (!fast) {
      for (const row of rows) {
        for (const block of row.querySelectorAll<HTMLElement>('p, li, pre, h1, h2, h3, h4, h5, h6')) {
  ...
  indexOf(el: HTMLElement): number {
    for (let i = 0; i < this.targets.length; i++) {
      if (this.targets[i].el === el) return i
    }
    return -1
  }
}
