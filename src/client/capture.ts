 ...
 */
import { toCanvas } from '../vendor/html-to-image/index.ts'
...
function rowOf(el: HTMLElement, flowList: HTMLElement): HTMLElement {
...
function buildFooterWrapper(
  ...
  if (brandSvg !== null) {
    const brand = brandSvg.cloneNode(true) as SVGElement
...
export async function captureRange(params: CaptureParams): Promise<CaptureOutput> {
  const { scrollport, flowList, startEl, endEl, brandSvg, themeBg } = params

  const rows = Array.from(flowList.children).filter((child): child is HTMLElement => child instanceof HTMLElement)
