...
export function findBrandSvg(): SVGElement | null {
...
function findBrandSvgElement(): SVGElement | null {
  const buttons = Array.from(document.querySelectorAll<HTMLElement>('button[class*="_brand"]'))
  ...
  return document.querySelector<SVGElement>(`svg[viewBox="${BRAND_VIEWBOX}"]`)
}

/** Copy resolved fill/stroke colors from the live SVG onto the clone. */
function resolvePresentationColors(source: SVGElement, clone: SVGElement): void {
  const sources = [source, ...Array.from(source.querySelectorAll<SVGElement>('*'))]
...
function neutralizeClipPaths(clone: SVGElement): void {
  for (const el of [clone, ...Array.from(clone.querySelectorAll<SVGElement>('*'))]) {
    if (el.getAttribute('clip-path') !== null) el.removeAttribute('clip-path')
  }
}
