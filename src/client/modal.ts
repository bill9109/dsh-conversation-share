...
import { chromeFontStack } from './icons.ts'
...
export class PreviewModal {
  ...
  show(output: CaptureOutput, title = ''): void {
    this.hide()
  ...
  private async copy(dataUrl: string): Promise<void> {
    try {
      const ItemClass = (window as unknown as { ClipboardItem?: new (items: Record<string, Blob>) => unknown }).ClipboardItem
      const clipboard = navigator.clipboard as unknown as { write: (items: unknown[]) => Promise<void> } | undefined
