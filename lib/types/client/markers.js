 ...
 * never cross.
 */
import { findComposerSeat } from "./dom.js";
import { chromeFontStack, gripIconSVG, themeColors } from "./icons.js";
import { SnapTargets } from "./snap-targets.js";
...
export class MarkerOverlay {
    ...
    activate(scrollport, flowList) {
        ...
        const onKeydown = (event) => {
            ...
            if (active === startH?.pill)
                handle = startH;
            else if (active === endH?.pill)
                handle = endH;
            if (handle === null)
                return;
            event.preventDefault();
    ...
    dispose() {
        ...
        if (this.flowList !== null)
            this.flowList.style.position = this.prevFlowListPosition;
        this.root?.remove();
        this.root = null;
    ...
    currentRange() {
        ...
        if (s === null)
            s = this.targets.nearestTop(this.rect.top + this.lineY(start));
        if (e === null)
            e = this.targets.nearestBottom(this.rect.top + this.lineY(end));
        if (s === null || e === null)
            return null;
    ...
    contentBox() {
        ...
        if (flowList === null)
            return { left: 12, width: Math.max(0, this.rect.width - 24) };
     ...
     */
    syncSnappedHandles() {
        if (this.disposed || this.root === null || this.scrollport === null)
            return;
        if (!this.handles.some(handle => handle.state === 'snapped'))
            return;
    ...
    sync() {
        ...
        if (start.state === 'free')
            start.freeY = clamp(start.freeY, 0, Math.max(0, endY));
        const startY = this.lineY(start);
        if (end.state === 'free')
            end.freeY = clamp(end.freeY, Math.max(0, startY), bottom);
    ...
    textNodeLineRanges(node, scrollTop) {
        ...
        for (let i = 0; i <= bounds.length; i++) {
            ...
            const rect = rects[i];
            if (rect !== undefined)
                result.push({ range: lineRange, top: rect.top + scrollTop, bottom: rect.bottom + scrollTop });
            prev = end;
        }
        return result;
    }
    ...
    applyDim(startEdge, endEdge) {
        ...
        for (const line of this.dimLines) {
            if (line.bottom <= startEdge || line.top >= endEdge)
                highlight.add(line.range);
        }
    ...
    syncDim() {
        ...
        const scrollTop = this.scrollport?.scrollTop ?? 0;
        if (range.startEdge === this.lastDimStartEdge && range.endEdge === this.lastDimEndEdge && scrollTop === this.lastDimScrollTop)
            return;
    ...
    flowOffsetOf(handle) {
        ...
        if (target === null || contentRoot === null)
            return 0;
